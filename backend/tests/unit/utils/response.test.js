...
describe('Response Utils', () => {
    ...
    describe('errorResponse', () => {
        it('should return error response with correct structure', () => {
            const res = createMockResponse();
            (0, response_1.errorResponse)(res, 'TEST_ERROR', 'Test error message', 400);
        ...
        it('should include error details when provided', () => {
            ...
            (0, response_1.errorResponse)(res, 'VALIDATION_ERROR', 'Validation failed', 400, details);
            expect(res.json).toHaveBeenCalledWith(expect.objectContaining({
    ...
    describe('createdResponse', () => {
        it('should return 201 status code', () => {
            const res = createMockResponse();
            (0, response_1.createdResponse)(res, { id: '123' });
            expect(res.status).toHaveBeenCalledWith(201);
        });
    });
    /**
    ...
    describe('Property: Response format is standardized', () => {
        ...
        it('error responses should always include success=false and error object', () => {
            fc.assert(fc.property(fc.string({ minLength: 1 }), fc.string({ minLength: 1 }), fc.integer({ min: 400, max: 599 }), (code, message, statusCode) => {
                const res = createMockResponse();
                (0, response_1.errorResponse)(res, code, message, statusCode);
                const jsonCall = res.json.mock.calls[0][0];
