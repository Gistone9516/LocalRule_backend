...
describe('QR Code Property Tests', () => {
    ...
    beforeEach(() => {
        ...
        qrService = new qr_service_1.QRService(mockPrisma);
    });
    afterEach(() => {
        mockPrisma.reset();
        jest.clearAllMocks();
    ...
    const createTestSpace = (id, adminId, data = {}) => {
        const space = {
            id,
            adminId,
    ...
    const createTestObject = (id, spaceId, objectTypeId) => {
        const obj = {
            id,
            spaceId,
            objectTypeId,
    ...
    const createTestRule = (id, objectId) => {
        const rule = {
            id,
            objectId,
            title: 'Test Rule',
    ...
    const createTestTrigger = (id, spaceId) => {
        const trigger = {
            id,
            spaceId,
            positionX: 3,
    ...
    describe('Property 31: QR lookup returns full space data', () => {
        it('should return space with all objects and rules', async () => {
            ...
            createTestAdmin(adminId);
            await fc.assert(fc.asyncProperty(generators_1.validSpaceArb, fc.integer({ min: 1, max: 5 }), fc.integer({ min: 1, max: 3 }), async (spaceData, numObjects, numRulesPerObject) => {
                mockPrisma.reset();
                createTestAdmin(adminId);
        ...
        it('should return space with all trigger zones', async () => {
            ...
            createTestAdmin(adminId);
            await fc.assert(fc.asyncProperty(generators_1.validSpaceArb, fc.integer({ min: 1, max: 5 }), async (spaceData, numTriggers) => {
                mockPrisma.reset();
                createTestAdmin(adminId);
                const spaceId = `space-${Date.now()}`;
                createTestSpace(spaceId, adminId, spaceData);
    ...
    describe('Property 32: QR scan increments counter', () => {
        it('should increment scan count by 1 on each scan', async () => {
            ...
            createTestAdmin(adminId);
            await fc.assert(fc.asyncProperty(generators_1.validSpaceArb, fc.integer({ min: 1, max: 10 }), async (spaceData, numScans) => {
                mockPrisma.reset();
                createTestAdmin(adminId);
                const spaceId = `space-${Date.now()}`;
                createTestSpace(spaceId, adminId, spaceData);
    ...
    describe('Property 33: Invalid QR returns error', () => {
        it('should throw QR_NOT_FOUND for non-existent codes', async () => {
            await fc.assert(fc.asyncProperty(generators_1.qrCodeArb, async (randomCode) => {
                mockPrisma.reset();
...
//# sourceMappingURL=qr.property.test.js.map