...
import { validTriggerArb, validRadiusArb, invalidRadiusArb } from '../helpers/generators';
    ...
    triggerZone: {
      ...
      findMany: jest.fn(({ where, include, orderBy }: any) => {
        let result = Array.from(triggers.values()).filter((t) => {
          if (where.spaceId && t.spaceId !== where.spaceId) return false;
          return true;
        });
        // Sort by createdAt
        ...
        result = result.map((trigger) => {
          ...
          if (include?.translations) {
            if (include.translations.where?.language) {
              t.translations = Array.from(translations.values()).filter(
                (tr) => tr.triggerId === trigger.id && tr.language === include.translations.where.language
              );
            } else {
              t.translations = Array.from(translations.values()).filter(
                (tr) => tr.triggerId === trigger.id
              );
            }
          }
          return t;
      ...
      count: jest.fn(({ where }: any) => {
        ...
        for (const t of triggers.values()) {
          if (where.spaceId && t.spaceId !== where.spaceId) continue;
          count++;
        }
...
describe('Trigger Property Tests', () => {
  ...
  beforeEach(() => {
    ...
    triggerService = new TriggerService(mockPrisma as any);
  });
  ...
  const setupTestData = (adminId: string, spaceId: string) => {
  ...
  describe('Property 24: Trigger creation stores all data', () => {
    it('should create trigger with all provided data', async () => {
      ...
      await fc.assert(
        fc.asyncProperty(validTriggerArb, async (triggerData) => {
          ...
          const result = await triggerService.create(spaceId, adminId, {
            ...
            triggerType: triggerData.triggerType as any,
            message: triggerData.message,
            autoCloseSeconds: triggerData.autoCloseSeconds,
            vibrationPattern: triggerData.vibrationPattern as any,
            soundEffect: triggerData.soundEffect as any,
          });
  ...
  describe('Property 25: Trigger list returns all triggers', () => {
    it('should return all created triggers in the list', async () => {
      ...
      await fc.assert(
        fc.asyncProperty(
          fc.array(validTriggerArb, { minLength: 1, maxLength: 20 }),
          async (triggersData) => {
            mockPrisma.triggers.clear();
    ...
    it('should only return triggers for the specified space', async () => {
      ...
      await fc.assert(
        fc.asyncProperty(
          validTriggerArb,
          validTriggerArb,
          async (triggerData1, triggerData2) => {
            mockPrisma.triggers.clear();
            ...
            const trigger1 = await triggerService.create(spaceId1, adminId, {
              ...
              triggerType: triggerData1.triggerType as any,
            ...
            await triggerService.create(spaceId2, adminId, {
              ...
              triggerType: triggerData2.triggerType as any,
  ...
  describe('Property 26: Trigger update persists changes', () => {
      ...
      await fc.assert(
        fc.asyncProperty(
          ...
          fc.record({
            ...
            radius: fc.option(validRadiusArb, { nil: undefined }),
          ...
          async (initialData, updateData) => {
            ...
            const created = await triggerService.create(spaceId, adminId, {
              ...
              triggerType: initialData.triggerType as any,
            ...
            const updated = await triggerService.update(created.id, adminId, {
              ...
              triggerType: updateData.triggerType as any,
    ...
    it('should preserve unchanged fields', async () => {
      ...
      await fc.assert(
        fc.asyncProperty(validTriggerArb, async (initialData) => {
          mockPrisma.triggers.clear();
  ...
  describe('Property 27: Trigger deletion cascades to translations', () => {
    ...
    it('should not affect translations of other triggers', async () => {
      ...
      await fc.assert(
        fc.asyncProperty(validTriggerArb, validTriggerArb, async (triggerData1, triggerData2) => {
          mockPrisma.triggers.clear();
          mockPrisma.translations.clear();
          ...
          const trigger2Trans = Array.from(mockPrisma.translations.values()).filter(
            (t) => t.triggerId === trigger2.id
          );
    ...
    it('should enforce limit of 20 triggers per space', async () => {
      ...
      await fc.assert(
        fc.asyncProperty(validTriggerArb, async (triggerData) => {
          ...
          await expect(
            triggerService.create(spaceId, adminId, {
              position: triggerData.position,
              radius: triggerData.radius,
              triggerType: triggerData.triggerType as any,
              message: triggerData.message,
    ...
    it('should reject invalid radius on update', async () => {
      ...
      await fc.assert(
        fc.asyncProperty(
          validTriggerArb,
          invalidRadiusArb,
          async (initialData, invalidRadius) => {
            mockPrisma.triggers.clear();
            ...
            await expect(
              triggerService.update(created.id, adminId, {
