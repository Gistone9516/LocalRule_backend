...
import { validSpaceArb } from '../helpers/generators';
      ...
      findUnique: jest.fn(({ where }: { where: { id?: string; email?: string } }) => {
        ...
        if (where.email) {
          for (const admin of admins.values()) {
            if (admin.email === where.email) return Promise.resolve(admin);
          }
        }
    ...
    space: {
      ...
      findMany: jest.fn(({ where, skip, take, orderBy }: any) => {
        let result = Array.from(spaces.values()).filter((s) => {
          if (where.adminId && s.adminId !== where.adminId) return false;
      ...
      count: jest.fn(({ where }: any) => {
        ...
        for (const s of spaces.values()) {
          if (where.adminId && s.adminId !== where.adminId) continue;
...
describe('Space Property Tests', () => {
  ...
  beforeEach(() => {
    ...
    spaceService = new SpaceService(mockPrisma as any);
  });
  ...
  describe('Property 9: Space list contains created spaces', () => {
    it('should return all created spaces in the list', async () => {
      ...
      await fc.assert(
        fc.asyncProperty(
          fc.array(validSpaceArb, { minLength: 1, maxLength: 5 }),
          async (spacesData) => {
  ...
  describe('Property 10: Space update persists changes', () => {
    it('should persist all updated fields', async () => {
      ...
      await fc.assert(
        fc.asyncProperty(validSpaceArb, validSpaceArb, async (originalData, updateData) => {
          mockPrisma.spaces.clear();
    ...
    it('should preserve unchanged fields during partial update', async () => {
      ...
      await fc.assert(
        fc.asyncProperty(validSpaceArb, fc.string({ minLength: 1, maxLength: 50 }), async (originalData, newName) => {
          mockPrisma.spaces.clear();
        ...
        fc.asyncProperty(
          ...
          async (spaceData, transition) => {
            ...
            if (transition.from !== 'draft') {
              ...
              await spaceService.updateStatus(created.id, adminId, transition.from as any);
            }

            // Perform the transition
            const updated = await spaceService.updateStatus(created.id, adminId, transition.to as any);
      ...
      await fc.assert(
        fc.asyncProperty(
          ...
          async (spaceData, transition) => {
            ...
            await expect(
              spaceService.updateStatus(created.id, adminId, transition.to as any)
            ).rejects.toThrow();
          }
        ),
        { numRuns: 10 }
  ...
  describe('Property 13: Free plan space limit enforced', () => {
    it('should enforce free plan limit of 1 space', async () => {
      ...
      await fc.assert(
        fc.asyncProperty(validSpaceArb, validSpaceArb, async (firstSpace, secondSpace) => {
          mockPrisma.spaces.clear();
          ...
          await expect(spaceService.create(adminId, secondSpace)).rejects.toThrow(
    ...
    it('should allow basic plan to create up to 5 spaces', async () => {
      ...
      await fc.assert(
        fc.asyncProperty(
          fc.array(validSpaceArb, { minLength: 6, maxLength: 6 }),
          async (spacesData) => {
            mockPrisma.spaces.clear();
            ...
            await expect(spaceService.create(adminId, spacesData[5])).rejects.toThrow(
              /Space limit exceeded/
            );
          }
        ),
    ...
    it('should allow enterprise plan unlimited spaces', async () => {
      ...
      await fc.assert(
        fc.asyncProperty(
          fc.array(validSpaceArb, { minLength: 10, maxLength: 10 }),
          async (spacesData) => {
            mockPrisma.spaces.clear();
