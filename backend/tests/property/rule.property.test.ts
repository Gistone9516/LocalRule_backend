...
import { validRuleArb, longTitleArb } from '../helpers/generators';
    ...
    localRule: {
      findUnique: jest.fn(({ where, include }: any) => {
        ...
        if (include?.translations) {
          result.translations = Array.from(translations.values()).filter(
            (t) => t.ruleId === rule.id
          );
        }
        if (include?.image) {
      ...
      findMany: jest.fn(({ where, include, orderBy }: any) => {
        let result = Array.from(rules.values()).filter((r) => {
          if (where.objectId && r.objectId !== where.objectId) return false;
          return true;
        });
        ...
        result = result.map((rule) => {
          ...
          if (include?.translations) {
            if (include.translations.where?.language) {
              r.translations = Array.from(translations.values()).filter(
                (t) => t.ruleId === rule.id && t.language === include.translations.where.language
              );
            } else {
              r.translations = Array.from(translations.values()).filter(
                (t) => t.ruleId === rule.id
      ...
      aggregate: jest.fn(({ where }: any) => {
        ...
        for (const r of rules.values()) {
          if (where.objectId && r.objectId !== where.objectId) continue;
          if (r.displayOrder > maxOrder) maxOrder = r.displayOrder;
        }
    ...
    ruleImage: {
      findUnique: jest.fn(({ where }: any) => {
        return Promise.resolve(
          Array.from(images.values()).find((i) => i.ruleId === where.ruleId) || null
        );
      }),
      delete: jest.fn(({ where }: any) => {
        for (const [imgId, img] of images.entries()) {
...
describe('Rule Property Tests', () => {
  ...
  beforeEach(() => {
    ...
    ruleService = new RuleService(mockPrisma as any);
  });
  ...
  const setupTestData = (adminId: string, spaceId: string, objectId: string) => {
  ...
  describe('Property 18: Rule creation stores all data', () => {
    it('should create rule with all provided data', async () => {
      ...
      await fc.assert(
        fc.asyncProperty(validRuleArb, async (ruleData) => {
          ...
          const result = await ruleService.create(objectId, adminId, {
            ...
            iconType: ruleData.iconType as any,
    ...
    it('should use default priority of 5 when not provided', async () => {
      ...
      await fc.assert(
        fc.asyncProperty(
          ...
          fc.constantFrom('prohibited', 'warning', 'tip', 'info'),
          async (title, description, iconType) => {
            const result = await ruleService.create(objectId, adminId, {
              title,
              description,
  ...
  describe('Property 20: Rule update persists changes', () => {
        ...
        fc.asyncProperty(
          ...
          async (initialData, updateData) => {
            ...
            const created = await ruleService.create(objectId, adminId, {
              ...
              iconType: initialData.iconType as any,
            ...
            const updated = await ruleService.update(created.id, adminId, {
              ...
              iconType: updateData.iconType as any,
    ...
    it('should preserve unchanged fields', async () => {
      ...
      await fc.assert(
        fc.asyncProperty(validRuleArb, async (initialData) => {
          mockPrisma.rules.clear();
  ...
  describe('Property 21: Rule deletion cascades to translations', () => {
    it('should delete associated translations when rule is deleted', async () => {
      ...
      await fc.assert(
        fc.asyncProperty(
          ...
          fc.uniqueArray(fc.constantFrom('en', 'ja', 'zh'), { minLength: 1, maxLength: 3 }),
          async (ruleData, languages) => {
            mockPrisma.rules.clear();
            ...
            const transBefore = Array.from(mockPrisma.translations.values()).filter(
              (t) => t.ruleId === rule.id
            );
            ...
            const transAfter = Array.from(mockPrisma.translations.values()).filter(
              (t) => t.ruleId === rule.id
            );
    ...
    it('should delete associated image when rule is deleted', async () => {
      ...
      await fc.assert(
        fc.asyncProperty(validRuleArb, async (ruleData) => {
          ...
          // Verify image exists
          const imageBefore = Array.from(mockPrisma.images.values()).find(
            (i) => i.ruleId === rule.id
          );
          ...
          const imageAfter = Array.from(mockPrisma.images.values()).find(
            (i) => i.ruleId === rule.id
          );
    ...
    it('should not affect translations of other rules', async () => {
      ...
      await fc.assert(
        fc.asyncProperty(validRuleArb, validRuleArb, async (ruleData1, ruleData2) => {
          mockPrisma.rules.clear();
          mockPrisma.translations.clear();
          ...
          const rule1 = await ruleService.create(objectId, adminId, {
            ...
            iconType: ruleData1.iconType as any,
          ...
          const rule2 = await ruleService.create(objectId, adminId, {
            ...
            iconType: ruleData2.iconType as any,
          ...
          const rule2Trans = Array.from(mockPrisma.translations.values()).filter(
            (t) => t.ruleId === rule2.id
          );
    ...
    it('should enforce limit of 10 rules per object', async () => {
      ...
      await fc.assert(
        fc.asyncProperty(validRuleArb, async (ruleData) => {
          ...
          await expect(
            ruleService.create(objectId, adminId, {
              title: ruleData.title,
              description: ruleData.description,
              iconType: ruleData.iconType as any,
              priority: ruleData.priority,
    ...
    it('should count rules per object independently', async () => {
      ...
      await fc.assert(
        fc.asyncProperty(validRuleArb, async (ruleData) => {
          ...
          await expect(
            ruleService.create(objectId1, adminId, {
              title: ruleData.title,
              description: ruleData.description,
              iconType: ruleData.iconType as any,
              priority: ruleData.priority,
  ...
  describe('Property 23: Rule title length validated', () => {
    it('should reject titles exceeding 30 characters on creation', async () => {
      ...
      await fc.assert(
        fc.asyncProperty(
          ...
          fc.string({ minLength: 1, maxLength: 200 }),
          fc.constantFrom('prohibited', 'warning', 'tip', 'info'),
          async (longTitle, description, iconType) => {
            // Ensure title is actually > 30 chars
            if (longTitle.length <= 30) return;
    ...
    it('should reject titles exceeding 30 characters on update', async () => {
      ...
      await fc.assert(
        fc.asyncProperty(validRuleArb, longTitleArb, async (initialData, longTitle) => {
          // Ensure title is actually > 30 chars
          if (longTitle.length <= 30) return;
          ...
          await expect(
            ruleService.update(created.id, adminId, {
              title: longTitle,
            })
          ).rejects.toThrow(/Title must not exceed 30 characters/);
    ...
    it('should accept titles with exactly 30 characters', async () => {
      ...
      await fc.assert(
        fc.asyncProperty(
          fc.stringOf(fc.constantFrom(...'abcdefghijklmnopqrstuvwxyz'), { minLength: 30, maxLength: 30 }),
          fc.string({ minLength: 1, maxLength: 200 }),
          fc.constantFrom('prohibited', 'warning', 'tip', 'info'),
          async (title30, description, iconType) => {
            mockPrisma.rules.clear();
    ...
    it('should accept titles with less than 30 characters', async () => {
      ...
      await fc.assert(
        fc.asyncProperty(
          ...
          fc.string({ minLength: 1, maxLength: 200 }),
          fc.constantFrom('prohibited', 'warning', 'tip', 'info'),
          async (shortTitle, description, iconType) => {
            mockPrisma.rules.clear();
